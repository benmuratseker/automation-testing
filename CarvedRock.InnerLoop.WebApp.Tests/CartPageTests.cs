...
public class CartPageTests(CustomWebAppFactory factory, ITestOutputHelper outputHelper) : IClassFixture<CustomWebAppFactory>
{
    [Fact]
    public async Task GetCartPageAsAnonymousRedirectToSignIn()
    {
    ...
    [Fact]
    public async Task GetLoggedInEmptyCartPage()
    {
        var client = factory.CreateClient(new WebApplicationFactoryClientOptions
        {
            AllowAutoRedirect = false,
        ...
        outputHelper.WriteLine(messageParagraph.OuterHtml);
    }
    ...
    private (string, double) GetCookieHeaderValueAndGrandTotal()
    {
        ...
        var productsToAdd = factory.SharedFixture.Faker.PickRandom(factory.SharedFixture.OriginalProducts, numberOfItemsToPick);
        ...
        foreach (var product in productsToAdd)
        {
            ...
            var cartItem =
                new CartItem(product.Id, quantityOfItem, "", "", 0, 0);
